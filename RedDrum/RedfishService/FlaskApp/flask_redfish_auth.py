...
class HTTPAuth(object):
    ...
    def rfAuthRequired(self, privilege=None,rdata=None):
        def rfAuthRequiredCallable(f ):
            @wraps(f)
            def decorated(*args, **kwargs):
                auth = request.authorization
                print("in rfAuthRequired--privilege: {}".format(privilege))
                ...
                if request.method != 'OPTIONS':
                    ...
                    if privilege is None:
                        pass
                    elif rdata.RedfishAllowAuthNone == 'true':
                        pass
                    else:
                        if request.scheme == 'http':
                            if rdata.RedfishAllowAuthenticatedAPIsOverHttp == 'false':
                                return Response('404-Not Found', 404)
                        # Checking if request authentication is Basic
                        if (found_token is not True) and rdata.RedfishAllowBasicAuthOverHttp == 'false':   
                            if request.headers.get('X_RM_HTTPS_ENABLED', None) == 'True':
                                parsed = list(urlparse(str(request.url)))
                                if 'http' in parsed:
                                    parsed[parsed.index('http')] = 'https'
                                https_url = urlunparse(parsed)
                                return Response('302-Use-HTTPS', 302, {'Location': str(https_url)})
                            else:
                                #https is not enabled
                                return Response('401-UnAuthorized', 401)
                        if (auth_token is not None) or (found_token  is not True):
                            if (auth_token is not None):
                                ...
                                if( authOk != '200'):
                                    ...
                                    #return( self.auth_token_error_callback(error=authOk))
                                    if authOk == '401':
                                        return Response('401-UnAuthorized', 401, {'WWW-Authenticate': 'Basic'})
                                    else:
                                        return Response('403-UnAuthenticated', 403)
                            else:
                                if auth:
                                ...
                                if( basicAuthOk != '200'):
                                    #return( self.auth_basic_error_callback(error=basicAuthOk))
                                    if basicAuthOk == '401':
                                        #print("In here")
                ...
                #             password = None
                #         ###print("basic auth: auth={}, pwd={}".format(auth,password))
                #         if( not self.authenticate(auth, password, privilege=privilege)):
                #             return( self.auth_basic_error_callback())
    ...
    '''
    #for redfish, we need to hook this to check if its token auth before trying basic auth
    def rfAuthRequired(self, f, privilege=None):
        @wraps(f)
        def decorated(*args, **kwargs):
            auth = request.authorization
            print("in rfAuthRequired--privilege: {}".format(privilege))
